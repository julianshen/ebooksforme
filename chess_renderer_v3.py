#!/usr/bin/env python3
"""
Chess Game Renderer v3 - Professional chess diagram generator
Uses high-quality embedded SVG pieces with move arrows and elegant design.
"""
...
# Embedded SVG piece data - refined Staunton-style pieces
# Based on CBurnett pieces (Wikimedia Commons, public domain)
PIECE_SVGS = {
...
class ChessRenderer:
    """Professional chess board renderer with high-quality piece images."""

    def __init__(self, piece_size: int = 90):
        self.piece_size = piece_size
        self.square_size = piece_size
        self.board_size = piece_size * 8

        # Refined color palette
        self.light_square = "#EBECD0"
        self.dark_square = "#779556"
        self.highlight_from = "#F5F682"  # Soft yellow
        self.highlight_to = "#B9CA43"    # Soft green
        self.arrow_color = "#FF0000"     # Red arrow
        self.check_highlight = "#FF6B6B"
    ...
    def _create_fallback_piece(self, key: str) -> Image.Image:
        """Create elegant fallback piece images."""
        img = Image.new('RGBA', (self.piece_size, self.piece_size), (0, 0, 0, 0))
        draw = ImageDraw.Draw(img)

        is_white = key[0] == 'w'
        fill = (255, 255, 255, 255) if is_white else (40, 40, 40, 255)
        outline = (60, 60, 60, 255) if is_white else (200, 200, 200, 255)
        piece_type = key[1]
        ...
        if piece_type == 'P':
            draw.ellipse([cx-8, cy-16, cx+8, cy], fill=fill, outline=outline, width=2)
            draw.ellipse([cx-12, cy, cx+12, cy+16], fill=fill, outline=outline, width=2)
        elif piece_type == 'N':
            draw.polygon([(cx, cy-20), (cx+16, cy+8), (cx-10, cy+10)], fill=fill, outline=outline, width=2)
            draw.ellipse([cx-12, cy+4, cx+12, cy+20], fill=fill, outline=outline, width=2)
        elif piece_type == 'B':
            draw.ellipse([cx-5, cy-20, cx+5, cy-10], fill=fill, outline=outline, width=2)
            draw.polygon([(cx, cy-12), (cx+15, cy+18), (cx-15, cy+18)], fill=fill, outline=outline, width=2)
        elif piece_type == 'R':
            draw.rectangle([cx-14, cy-10, cx+14, cy+16], fill=fill, outline=outline, width=2)
            draw.rectangle([cx-11, cy-18, cx+11, cy-10], fill=fill, outline=outline, width=2)
        elif piece_type == 'Q':
            draw.ellipse([cx-16, cy-4, cx+16, cy+18], fill=fill, outline=outline, width=2)
            for dx in [-12, -4, 4, 12]:
                draw.ellipse([cx+dx-4, cy-22, cx+dx+4, cy-10], fill=fill, outline=outline, width=2)
        elif piece_type == 'K':
            draw.ellipse([cx-14, cy-4, cx+14, cy+18], fill=fill, outline=outline, width=2)
            draw.rectangle([cx-3, cy-22, cx+3, cy-10], fill=fill, outline=outline, width=2)
            draw.rectangle([cx-10, cy-18, cx+10, cy-14], fill=fill, outline=outline, width=2)

        return img

    def _draw_arrow(self, draw: ImageDraw.Draw, x1: int, y1: int, x2: int, y2: int, color: str = "#FF0000"):
        ...
        # Offset for width
        perp_x = -math.sin(angle) * body_width
        perp_y = math.cos(angle) * body_width

        body_points = [
            (x1 + perp_x, y1 + perp_y),
            (x2 - head_len * math.cos(angle) + perp_x, y2 - head_len * math.sin(angle) + perp_y),
            (x2 - head_len * math.cos(angle) - perp_x, y2 - head_len * math.sin(angle) - perp_y),
            (x1 - perp_x, y1 - perp_y),
        ]
    ...
    def render_board(self,
                     ...
                     show_arrow: bool = True) -> Image.Image:
        """Render a chess board position with optional move arrow."""

        coord_margin = 28 if show_coordinates else 0
        title_height = 48 if title else 0
        caption_height = 28 if caption else 0
        padding = 16
        ...
        try:
            ...
            font_caption = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 14)
            font_coord = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 12)
        except:
        ...
        if title:
            ...
            draw.text(((total_width - tw) // 2, padding), title,
                     fill='#2c3e50', font=font_title)

        board_x = padding + coord_margin
        board_y = padding + title_height + coord_margin

        # Get arrow coordinates if needed
        arrow_from = None
        arrow_to = None
        if show_arrow and highlight_from and highlight_to:
            from_sq = chess.parse_square(highlight_from)
            to_sq = chess.parse_square(highlight_to)
            ...
            arrow_from = (board_x + from_col * self.square_size + self.square_size // 2,
                         board_y + from_row * self.square_size + self.square_size // 2)
            arrow_to = (board_x + to_col * self.square_size + self.square_size // 2,
                       board_y + to_row * self.square_size + self.square_size // 2)
        ...
        if arrow_from and arrow_to:
            self._draw_arrow(draw, arrow_from[0], arrow_from[1],
                           arrow_to[0], arrow_to[1], self.arrow_color)

        # Coordinates
        if show_coordinates:
            for i in range(8):
                file_label = chr(ord('a') + (7 - i if flip else i))
                x = board_x + i * self.square_size + self.square_size // 2 - 4
                y_bottom = board_y + self.board_size + 6
                is_light = i % 2 == 1
                color = '#EBECD0' if is_light else '#779556'
                draw.text((x, y_bottom), file_label, fill=color, font=font_coord)

                rank_label = str(i + 1 if flip else 8 - i)
                x_left = board_x - 18
                y = board_y + i * self.square_size + self.square_size // 2 - 6
                is_light = i % 2 == 0
                color = '#EBECD0' if is_light else '#779556'
        ...
        # Caption
        if caption:
            y_caption = board_y + self.board_size + coord_margin + 8
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Chess Game Renderer v3')
    ...
    parser.add_argument('--size', '-s', type=int, default=90, help='Piece size')
