...
6. **熱門股票必須動態**：熱門股票區塊的股票清單必須根據當日市場熱度決定，不要固定只放 NVDA/TSLA/AAPL 等。請參考當日 Yahoo Finance Trending/Most Active 股票，挑選 8-12 隻真正有話題、有成交量的股票。可以包含當日暴漲暴跌的中小型股。
7. **Prompt 必須包含當日熱門股票參考**：在生成熱門股票區塊時，請參考以下當日實際熱門股票數據（來自 stockanalysis.com 和 Yahoo Finance）：
   - Most Active: NOTV (+91.80%), SNBR (+36.42%), SMTK (+27.72%), NVDA (-6.20%), RMSG (+66.36%), NOK (-13.48%), BCDA (+12.08%), ZCMD (-28.18%), INTC (-11.28%), KEEL (-13.49%)
   - Top Gainers: CRNG (+400%), SCAG (+194.58%), SPHL (+123.94%), NOTV (+91.80%), RMSG (+66.36%), STI (+57.29%), BGMS (+41.51%), SUGP (+38.97%)
   - 請從這些實際數據中挑選 8-12 隻最有話題的股票，不要自己想像其他股票
...
### 3. id="stocks" - 熱門股票
- 熱門股票清單：根據當日實際熱門股票數據（見上方規則第7點）挑選 8-12 隻，必須包含當日真正有話題、高成交量的股票，不限於大型科技股，可包含中小型股
